...
template <int kMatrixX, int kMatrixY, int kBlockX, int kBlockY, int kBlockZ, int kSegLen, int kThreadSpan = 2, typename T>
__global__ void scan(const T * __restrict__ src, T * __restrict__ dst)
{
    ...
    BlockPrefixCallbackOp yCallback[kMatrixX / kBlockX][kThreadSpan];

    #pragma unroll
    for (int yi = 0; yi < kMatrixY / kBlockY / kThreadSpan; ++yi)
    {
        BlockPrefixCallbackOp xCallback[kThreadSpan];

        #pragma unroll
        for (int xi = 0; xi < kMatrixX / kBlockX / kThreadSpan; ++xi)
        {
            int gx = (xi * kBlockX * kThreadSpan) + (warpIdx * kSegLen + laneIdx) * kThreadSpan;
            int gy = (yi * kBlockY * kThreadSpan) + (warpIdy * (kWarpThreads / kSegLen) + laneIdy) * kThreadSpan;

            for (int y = 0; y < kThreadSpan; ++y)
            {
                for (int x = 0; x < kThreadSpan; ++x)
                {
                    const int gi = (gy + y) * kMatrixX + (gx + x);
                    input[y][x] = src[gi];
                }
            }

            scan.InclusiveScan(input, input, scanOp, xCallback, ndmamba::kHorizontal);
            scan.InclusiveScan(input, input, scanOp, yCallback[xi], ndmamba::kVertical);
...
int main()
{
    ...
    constexpr dim3 kMatrixSize(64, 64);
...
//        auto d = std::uniform_int_distribution(1, 100000);
    ...
    scan<kMatrixSize.x, kMatrixSize.y, kBlock.x, kBlock.y, kBlock.z, kBlock.y><<<1, kBlock>>>(
    ...
    printf("seg pscan ");
    std::cout << hRes(kMatrixSize.y - 1, kMatrixSize.x - 1) << '\n';
