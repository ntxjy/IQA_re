...
try:
    ...
    try:
        ...
        print("No mamba2 installed")


ADDITIONAL_NORM_FOR_2D = False
SPERATE_HW = True
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        if ADDITIONAL_NORM_FOR_2D and use_v2d:
            print("feature_size:", feature_size)
            # self.v2d_out_norm, _ = self.get_outnorm(forward_type, self.d_inner, channel_first)
            # self.v2d_out_norm = nn.LayerNorm([feature_size, feature_size])
            from .network_utils import AffineFirstLayerNorm
            self.v2d_out_norm = AffineFirstLayerNorm(affine_shape=[feature_size, feature_size],
                                                     normalized_shape=[self.d_inner, feature_size, feature_size])
        else:
        ...
        BACKEND_TYPES = dict(
            ...
            v02="mamba",
            v03="oflex",
            v04=None,  # selective_scan_backend="oflex", scan_mode="cross2d"
            v05=None,
        ...
        FORWARD_TYPES = dict(
            ...
            v052d3=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode=3,
                           selective_scan_backend=self.backend,), # debug
            # ===============================
            v2=partial(self.forward_corev2, force_fp32=(not self.disable_force32),
                       selective_scan_backend=self.backend),
            v3=partial(self.forward_corev2, force_fp32=False,
                       selective_scan_backend=self.backend),
        )
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        assert selective_scan_backend in [None, "oflex", "mamba", "torch", "v2dmamba"]
        ...
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True, HH=None, WW=None, DD=None):
            if SPERATE_HW and HH != WW:
                BB, KK, NN, LL = B.shape
                u = u.reshape(BB, KK, DD, LL)
                A = A.reshape(KK, DD, NN)
                D = D.reshape(KK, DD)
                delta = delta.reshape(BB, KK, DD, LL)
                delta_bias = delta_bias.reshape(KK, DD)
                ys = []
                for i in range(KK):
                    u_i = u[:, i].contiguous()
                    delta_i = delta[:, i].contiguous()
                    A_i = A[i].contiguous()
                    B_i = B[:, [i]].contiguous()
                    C_i = C[:, [i]].contiguous()
                    D_i = D[i].contiguous()
                    delta_bias_i = delta_bias[i].contiguous()
                    HH_i = HH if i == 0 or i == 2 else WW
                    WW_i = WW if i == 0 or i == 2 else HH
                    y = selective_scan_fn(u_i, delta_i, A_i, B_i, C_i, D_i, delta_bias_i, delta_softplus, ssoflex,
                                      backend=selective_scan_backend, HH=HH_i, WW=WW_i)
                    ys.append(y)
                return torch.stack(ys, dim=1)
            else:
                return selective_scan_fn(u, delta, A, B, C, D, delta_bias, delta_softplus, ssoflex,
                                         backend=selective_scan_backend, HH=HH, WW=WW)
        ...
        if _scan_mode == -1:
            ...
            def scan_rowcol(
                ...
                ys: torch.Tensor = selective_scan(
                    xs, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus, HH=XH, WW=XW,
                ).view(_B, 2, -1, _L)
                return ys
            ...
            if self.v2d_out_norm is not None:
                # ys = ys.reshape(B * K, ys.shape[2], H, W)
                # ys = self.v2d_out_norm(ys)
                # ys = ys.reshape(B, K, -1, H, W)
                ys = self.v2d_out_norm(ys)
...
class VSSM(nn.Module):
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        supported_ops={
            ...
            "prim::PythonOp.SelectiveScanCuda": partial(selective_scan_flop_jit, backend="prefixsum",
                                                        verbose=verbose, use_v2d=self.use_v2d),
        }
...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", **kwargs):
        ...
        # print("kwargs in Backbone_VSSM", kwargs)
