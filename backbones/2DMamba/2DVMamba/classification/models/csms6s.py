...
class SelectiveScanCuda(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, oflex=True, backend=None,
        ...
        ctx.HH = HH
        ctx.WW = WW
        # print(f"SelectiveScanCuda.forward: backend={backend}, HH={HH}, WW={WW}")
        if backend == "oflex":
        ...
        elif backend == "v2dmamba":
            out, x, *rest = v2dmamba_scan.fwd(u, delta, A, B, C, D, None, delta_bias, delta_softplus, HH, WW)
        else:
            raise NotImplementedError

        if DEBUG_NAN and torch.any(torch.isnan(out)):
            print(f"SelectiveScanCuda.forward: nan detected in output. u={u.shape}, delta={delta.shape}, A={A.shape}, B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}")
            os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
            # save u, delta, A, B, C, D, delta_bias, delta_softplus, HH, WW to a file
            torch.save({
                "u": u,
                "delta": delta,
                "A": A,
                ...
                "HH": HH,
                "WW": WW,
            }, "/gpfs/scratch/jingwezhang/debug/nan_detected_fwd.pt")
            raise RuntimeError(
                f"SelectiveScanCuda.forward: nan detected in output. "
                f"u={u.shape}, delta={delta.shape}, A={A.shape}, B={B.shape}, "
                f"C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}")

        if DEBUG_INF and torch.any(torch.isinf(out)):
            print(f"SelectiveScanCuda.forward: inf detected in output. u={u.shape}, delta={delta.shape}, A={A.shape}, B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}")
            os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
            # save u, delta, A, B, C, D, delta_bias, delta_softplus, HH, WW to a file
            torch.save({
                "u": u,
                ...
                "HH": HH,
                "WW": WW,
            }, "/gpfs/scratch/jingwezhang/debug/inf_detected_fwd.pt")
            raise RuntimeError(
                f"SelectiveScanCuda.forward: inf detected in output. "
                f"u={u.shape}, delta={delta.shape}, A={A.shape}, B={B.shape}, "
                f"C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}")
    ...
    def backward(ctx, dout, *args):
        ...
        if DEBUG_NAN and torch.any(torch.isnan(dout)):
            os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
            # save u, delta, A, B, C, D, delta_bias, delta_softplus, HH, WW, dout to file
            torch.save({
                "u": u,
                "delta": delta,
                "A": A,
                ...
                "dout": dout,
                "x": x,
            }, "/gpfs/scratch/jingwezhang/debug/nan_detected_bwd.pt")
            raise RuntimeError(
                f"SelectiveScanCuda.backward: nan detected in dout. u={u.shape}, delta={delta.shape}, A={A.shape}, "
                f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")

        if DEBUG_INF and torch.any(torch.isinf(dout)):
            os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
            # save u, delta, A, B, C, D, delta_bias, delta_softplus, HH, WW, dout to file
            torch.save({
                "u": u,
                ...
                "dout": dout,
                "x": x,
            }, "/gpfs/scratch/jingwezhang/debug/inf_detected_bwd.pt")
            raise RuntimeError(
                f"SelectiveScanCuda.backward: inf detected in dout. u={u.shape}, delta={delta.shape}, A={A.shape}, "
                  f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")
        ...
        grad_dict = {"du": du,
                ...
                "dC": dC,
                "dD": dD,
                "ddelta_bias": ddelta_bias,}

        if DEBUG_NAN:
            flag = False
            for grad_name, grad in grad_dict.items():
                if torch.any(torch.isnan(grad)):
                    flag = True
                    print(f"SelectiveScanCuda.backward: nan detected in {grad_name}. "
                          f"u={u.shape}, delta={delta.shape}, A={A.shape}, "
                          f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")
            if flag:
                os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
                # save u, delta, A, B, C, D, delta_bias, delta_softplus, HH, WW, dout to file
                ...
                raise RuntimeError(
                    f"SelectiveScanCuda.backward: nan detected in {grad_name}. "
                    f"u={u.shape}, delta={delta.shape}, A={A.shape}, "
                    f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")
        ...
        if DEBUG_INF:
            ...
            for grad_name, grad in grad_dict.items():
                if torch.any(torch.isinf(grad)):
                    flag = True
                    print(f"SelectiveScanCuda.backward: inf detected in {grad_name}. "
                          f"u={u.shape}, delta={delta.shape}, A={A.shape}, "
                          f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")
            if flag:
                os.makedirs("/gpfs/scratch/jingwezhang/debug/", exist_ok=True)
                ...
                raise RuntimeError(
                    f"SelectiveScanCuda.backward: inf detected in {grad_name}. "
                    f"u={u.shape}, delta={delta.shape}, A={A.shape}, "
                    f"B={B.shape}, C={C.shape}, D={D.shape}, delta_bias={delta_bias.shape}, dout={dout.shape}")
...
def selective_scan_fn(
    ...
    return fn(u, delta, A, B, C, D, delta_bias, delta_softplus, oflex, backend, HH, WW)
...
def flops_selective_scan_fn(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_complex=False, use_v2d=False):
    """
    u: r(B D L)
    delta: r(B D L)
    A: r(D N)
...
def flops_selective_scan_ref(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_Group=True,
                             with_complex=False, use_v2d=False):
    """
    u: r(B D L)
    delta: r(B D L)
    A: r(D N)
...
def selective_scan_flop_jit(inputs, outputs, backend="prefixsum", verbose=True, use_v2d=False):
    if verbose:
        print_jit_input_names(inputs)
    flops_fn = flops_selective_scan_ref if backend == "naive" else flops_selective_scan_fn
    B, D, L = inputs[0].type().sizes()
    N = inputs[2].type().sizes()[1]
    flops = flops_fn(B=B, L=L, D=D, N=N, with_D=True, with_Z=False, use_v2d=use_v2d)
    return flops
