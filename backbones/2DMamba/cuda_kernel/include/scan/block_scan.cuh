...
private:
     ...
     */
    static const SegBlockScanAlgorithm SAFE_ALGORITHM =
    ...
    typedef BlockScanRaking<T, BLOCK_DIM_X, BLOCK_DIM_Y, BLOCK_DIM_Z, (SAFE_ALGORITHM ==
                                                                       BLOCK_SCAN_RAKING_MEMOIZE), PTX_ARCH> Raking;
    ...
    static const SegBlockScanAlgorithm SAFE_ALGORITHM = BLOCK_SCAN_WARP_SCANS;

    typedef SegBlockScanWarpScans<T, kSegLen, BLOCK_DIM_X, BLOCK_DIM_Y, BLOCK_DIM_Z, PTX_ARCH> WarpScans;

    /// Define the delegate type for the desired algorithm
    typedef WarpScans InternalBlockScan;

    /// Shared memory storage layout type for BlockScan
    typedef typename InternalBlockScan::TempStorage InternalBlockScanTempStorage;
    ...
    InternalBlockScanTempStorage & temp_storage;
    ...
    __device__ __forceinline__ InternalBlockScanTempStorage & PrivateStorage()
    {
        __shared__ InternalBlockScanTempStorage private_storage;
...
public:
    ...
    struct TempStorage : cub::Uninitialized<InternalBlockScanTempStorage>
    {
    };
    ...
    __device__ __forceinline__ void constructorStaticAssert()
    {
        static_assert(BLOCK_THREADS % CUB_WARP_THREADS(PTX_ARCH) == 0);
    }
    ...
    {
        // Reduce consecutive thread items in registers
        T thread_prefix = cub::internal::ThreadReduce(input, scan_op);

        // Exclusive thread block-scan
        ExclusiveScan(thread_prefix, thread_prefix, initial_value, scan_op);

        // Exclusive scan in registers with prefix as seed
        cub::internal::ThreadScanExclusive(input, output, scan_op, thread_prefix);
    }
    ...
    {
        // Reduce consecutive thread items in registers
        T thread_partial = cub::internal::ThreadReduce(input, scan_op);

        // Exclusive thread block-scan
        ExclusiveScan(thread_partial, thread_partial, scan_op);

        // Exclusive scan in registers with prefix
        cub::internal::ThreadScanExclusive(input, output, scan_op, thread_partial, (linear_tid != 0));
    }
            ...
            T               (& output)[ITEMS_PER_THREAD])    ///< [out] Calling thread's output items (may be aliased to \p input)
    {
        ...
        else
        {
            ...
            cub::internal::ThreadScanInclusive(input, output, scan_op, thread_prefix, (linear_tid != 0));
        }
    }
    ...
    __device__ __forceinline__ void ExclusiveScan(
            ...
            ScanDir scanDir)
    {
        InternalBlockScan(temp_storage).ExclusiveScan(input, output, scan_op, scanDir);
    }
    ...
    __device__ __forceinline__ void ExclusiveScan(T input,
                                                  ...
                                                  ScanDir scanDir)
    {
        InternalBlockScan(temp_storage).ExclusiveScan(input, output, scan_op, block_prefix_callback_op, scanDir);
    }
    ...
    {
        InternalBlockScan(temp_storage).InclusiveScan(input, output, scan_op, scanDir);
    }
    ...
    __device__ __forceinline__ void InclusiveScan(
            ...
            ScanDir scanDir)             ///< [in] Scan direction
    {
        InternalBlockScan(temp_storage).InclusiveScan(input, output, scan_op, block_aggregate, scanDir);
    }
    ...
    __device__ __forceinline__ void InclusiveScan(
            ...
            ScanDir scanDir)             ///< [in] Scan direction
    {
        InternalBlockScan(temp_storage).InclusiveScan(input, output, scan_op, block_prefix_callback_op, scanDir);
    }
    ...
    template <
            ...
            typename ScanOp>
    __device__ __forceinline__ void InclusiveScan(
            T               (& input)[LENGTH][LENGTH],     ///< [in] Calling thread's input items
            T               (& output)[LENGTH][LENGTH],    ///< [out] Calling thread's output items (may be aliased to \p input)
            ScanOp scan_op,                        ///< [in] Binary scan functor
            ScanDir scanDir)
    {
        if constexpr (LENGTH == 1)
        {
            InclusiveScan(input[0][0], output[0][0], scan_op, scanDir);
            return;
        }

        // Reduce consecutive thread items in registers
        T thread_prefixes[LENGTH];
        ThreadReduce(input, thread_prefixes, scan_op, scanDir);

        #pragma unroll
        for (int i = 0; i < LENGTH; ++i)
        {
            // Exclusive thread block-scan
            ExclusiveScan(thread_prefixes[i], thread_prefixes[i], scan_op, scanDir);
        }

        const int warp_id = linear_tid / InternalBlockScan::WARP_THREADS;
        ...
        {
            if (warp_id % InternalBlockScan::kWarpsAcrossBlockDimX == 0 && lane_id % InternalBlockScan::kWarpDimX == 0)
            {
                // Exclude threads lying in first column in this thread block.
                // (Considering tiled square warp layout, threads are not linearly spaced in row major).
                applyPrefix = false;
            }
        }
        else if (scanDir == kVertical)
        {
            if (warp_id < InternalBlockScan::kWarpsAcrossBlockDimX && lane_id < InternalBlockScan::kWarpDimX)
            {
                // Exclude threads lying in first row in this thread block.
                // (Considering tiled square warp layout, threads are not linearly spaced in row major).
                applyPrefix = false;
            }
        }
        ...
        {
            if ((warp_id + 1) % InternalBlockScan::kWarpsAcrossBlockDimX == 0 &&
                (lane_id + 1) % InternalBlockScan::kWarpDimX == 0)
            {
                // Exclude threads lying in last column in this thread block.
                applyPrefix = false;
            }
        }
        else if (scanDir == kVerticalReversed)
        {
            if (InternalBlockScan::WARPS - InternalBlockScan::kWarpsAcrossBlockDimX <= warp_id &&
                InternalBlockScan::WARP_THREADS - InternalBlockScan::kWarpDimX <= lane_id)
            {
                // Exclude threads lying in last row in this thread block.
                applyPrefix = false;
            }
        }
        else
        ...
        // Inclusive scan in registers with prefix as seed (threads at first row or first column does not seed).
    ...
    __device__ __forceinline__ void InclusiveScan(
            ...
            T               (& output)[LENGTH][LENGTH],    ///< [out] Calling thread's output items (may be aliased to \p input)
            ScanOp scan_op,                        ///< [in] Binary scan functor
            BlockPrefixCallbackOp (& block_prefix_callback_op)[LENGTH],      ///< [in-out] <b>[<em>warp</em><sub>0</sub> only]</b> Call-back functor for specifying a block-wide prefix to be applied to the logical input sequence.
            ScanDir scanDir)
    {
        if (LENGTH == 1)
        {
            InclusiveScan(input[0][0], output[0][0], scan_op, block_prefix_callback_op[0], scanDir);
            return;
        }
        ...
        {
            // Exclusive thread block-scan
            ExclusiveScan(thread_prefixes[i], thread_prefixes[i], scan_op, block_prefix_callback_op[i], scanDir);
        }
