                     ...
                     T * __restrict__ vertAgg)
{
    ...
    scan.InclusiveScan(input, input, scanOp, ndmamba::kHorizontalReversed);
    scan.InclusiveScan(input, input, scanOp, ndmamba::kVerticalReversed);
...
int main()
{
    ...
    scan<kBlock.x, kBlock.y, kBlock.z, kSegLen, float><<<1, kBlock>>>(
