...
from configs.train_finetune_config import get_arguments
...
def main():
    ...
    if args.restore_from == RESTORE_FROM:  #without pretraining
        start_iter = 0
    ...
    opts = torch.optim.Adam(itertools.chain(model.parameters(),StyleFilter.parameters()), lr=args.learning_rate)    #optimizer for both networks
    ...
    for i_iter in tqdm(range(start_iter, args.num_steps)): 
        ...
        if i_iter >= args.num_steps_stop - 1:  #save model in the end
            ...
            torch.save(StyleFilter.state_dict(), osp.join(args.snapshot_dir, args.file_name + 'style' + str(args.num_steps_stop) + '.pth'))
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:       #checkpoint
            ...
            torch.save({
               'state_dict':StyleFilter.state_dict(),
               'train_iter':i_iter,
               'args':args
            },osp.join(args.snapshot_dir, run_name)+'_Style'+str(i_iter)+'.pth')
        ...
        if i_iter % args.loss_save_step == 0 and i_iter != 0:      #save best
            ...
            if val_psnr >= old_val_psnr:
                ...
                torch.save(StyleFilter, args.snapshot_dir + '/style_best_all')
                print('model saved')
