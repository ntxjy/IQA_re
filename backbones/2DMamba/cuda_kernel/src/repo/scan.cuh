...
struct WarpReverseScan
{
    ...
    unsigned int member_mask;  // 0xffffffffU for 32-sized warps
        ...
        ScanOpT scan_op)            ///< [in] Binary scan operator
    {
        ...
        #pragma unroll
        for (int STEP = 0; STEP < STEPS; STEP++)  // step == 5 (for warpsize 32)
        {
...
struct BlockReverseScan
{
        ...
        BlockPostfixCallbackOp & block_postfix_callback_op)     ///< [in-out] <b>[<em>warp</em><sub>0</sub> only]</b> Call-back functor for specifying a thread block-wide postfix to be applied to all inputs.
    {
        if (WARP_SYNCHRONOUS)
        {
            ...
            exclusive_output =
                linear_tid == BLOCK_THREADS - 1 ? block_postfix : scan_op(block_postfix, exclusive_output);
        }
        else
        {
            printf("not warp synchronous\n");
