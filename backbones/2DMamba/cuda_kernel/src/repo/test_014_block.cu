                     ...
                     T * __restrict__ vertAgg)
{
    ...
    T threadHoriAgg;
    scan.InclusiveScan(input, input, scanOp, threadHoriAgg, mamband::kHorizontal);

    T threadVertAgg;
    scan.InclusiveScan(input, input, scanOp, threadVertAgg, mamband::kVertical);

    dst[gi] = input;
    horiAgg[gi] = threadHoriAgg;
    vertAgg[gi] = threadVertAgg;
}
